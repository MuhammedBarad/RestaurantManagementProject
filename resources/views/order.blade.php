    ...
    <div class="container">
        ...
        <div class="row">
                    ...
                    <div class="order-cart-item d-flex justify-content-around">
                        ...
                        <div class="food-desc-wrapper">
                            ...
                            <div class="mobile d-flex pt-2">
                                <p class="price">TL {{ number_format($orderItem->menu->price, 2) }}</p>
                                <p class="quantity">x{{ $orderItem->quantity }}</p>
                                <p class="cart-item-total">TL {{ number_format($orderItem->menu->price * $orderItem->quantity, 2) }}</p>
                            </div>
                            <p class="text-muted desktop">{{ $orderItem->menu->description }}</p>
                        </div>
                        <p class="price desktop">TL {{ number_format($orderItem->menu->price, 2) }}</p>
                        <p class="quantity desktop">x{{ $orderItem->quantity }}</p>
                        <p class="cart-item-total desktop">TL {{ number_format($orderItem->menu->price * $orderItem->quantity, 2) }}</p>
                    </div>
                    <hr>
                @endforeach
                </div>
            ...
            <div class="order-summary p-4 col-lg-3 offset-lg-1 col-md-12">
                ...
                <div class="d-flex justify-content-between">
                    <h6>Subtotal</h6>
                    <p>TL {{ $subtotal = $activeOrder->getSubtotal() }}</p>
                </div>
                <div class="d-flex justify-content-between">
                    <h6>Discount</h6>
                    <p>-TL {{ $discount = $activeOrder->getDiscount($subtotal) }}</p>
                </div>
                <div class="d-flex justify-content-between">
                    <h6>Tax (6%)</h6>
                    <p>TL {{ $tax = $activeOrder->getTax($subtotal, $discount) }}</p>
                </div>
                <hr>
                <div class="d-flex justify-content-between">
                    <h6>Total</h6>
                    <p>TL {{ $activeOrder->getTotal($subtotal, $discount, $tax) }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                @endif
                    ...
                    <td>TL {{ $order->getTotalFromScratch() }}</td>
                    <td>
                        @if ($order->completed)
                            <div class="px-3 alert alert-success">
                                Fulfilled
